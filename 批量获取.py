...
def process_movie(url):
    ...
    if episodes_tag:
        ...
        episodes_value = next_sibling.strip() if next_sibling and next_sibling.name is None else ''
...
with open('data.csv', mode='w', newline='', encoding='utf-8') as csvfile:
    ...
    # 使用 ThreadPoolExecutor 来并发处理请求和写入数据
    with ThreadPoolExecutor(max_workers=10) as executor:
        ...
        for future in futures:
            ...
            if movie_info:
                ...
                print(f"电影《{movie_info['标题']}》信息已成功写入到 data.csv 文件中。")
