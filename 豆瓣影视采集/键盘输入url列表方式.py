# 此脚本数据url来源为 从键盘输入，每个url使用英文逗号隔开
import requests
...
# 从键盘输入 URL 列表，用英文逗号隔开
input_urls = input("请输入豆瓣电影的URL列表，用英文逗号隔开：")
urls = [url.strip() for url in input_urls.split(',') if url.strip()]

# 使用 ThreadPoolExecutor 来并发处理请求和打印数据，线程数：10
